...
func AddTeam(w http.ResponseWriter, r *http.Request){
	if r.Method == "POST"{
		...
		if err := team_player.InsertTeam(team); err != nil{
...
func GetTeam(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		team.TeamName = name
		team, err := team_player.GetTeam(team)
		if err != nil{
...
func DeleteTeam(w http.ResponseWriter, r *http.Request){
	if r.Method == "DELETE" {
		...
		team.TeamName = name
		if err := team_player.RemoveTeam(team); err != nil{
...
func AddPlayer(w http.ResponseWriter, r *http.Request){
	if r.Method == "PUT"{
		...
		if err := team_player.AddPlayerToTeam(team,player); err != nil{
			errore := map[string]string{
				"error":fmt.Sprintf("%v",err),
			}
...
func UpdatePlayerPrice(w http.ResponseWriter, r *http.Request){
	if r.Method == "PUT"{
		...
		if err := team_player.UpdatePlayer(player);err!=nil{
...
func RemovePlayer(w http.ResponseWriter, r *http.Request){
	if r.Method == "PUT"{
		...
		player.PlayerName = namePlayer
		if err := team_player.RemovePlayerToTeam(team,player); err != nil{
...
func ChangePlayer(w http.ResponseWriter, r *http.Request){
	if r.Method == "PUT"{
		...
		if err := team_player.ScambiaPlayer(team1,team2,player1,player2); err != nil{
			utils.ResponseJSON(w, "Operazione non consentita", http.StatusBadRequest)
			return
		}
		res := map[string]string{
